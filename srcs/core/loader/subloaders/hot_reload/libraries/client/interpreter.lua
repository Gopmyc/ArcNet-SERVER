local function wrapVMFunction(vmFunc)
	return setmetatable({
		call = function(self, ...)
			return vmFunc(...)
		end,
		setfenv = function(self, env)
			vmFunc.__vm_func.env = env
			return self
		end,
		getfenv = function(self)
			return vmFunc.__vm_func.env
		end
	}, {
...
local interpreter = (function()
    local fev = getfenv or function() return _ENV end
    ...
    function a4(bR, bu, b2)
    	...
    	local function bY(...)
    		...
    		local ok, nResults, results = pcall(bs, bm, ...)
    		if ok then
...
function LIBRARY:Interpreter(sCode)
	assert(isstring(sInput), "[LIBRARY INTERPRETER] Expected string for input")

	return interpreter(sCode)
end

function LIBRARY:Interpret(sByteCode)
	if SERVER then return end

	local _, ReturnValue	= xpcall(function(sByteCode)
		assert(isstring(sByteCode), "[SUBLOADER:VM:INTERPRETER] 'Interpret' method exepted a string for argument #1")
		return interpreter(sByteCode)
	end, function(sErr)
		MsgC(
